...
export const SCHEMAS: Record<CollectionName, Schema> = {
  ...
  sensors: {
    type: { type: 'enum', values: ['water_level', 'rainfall', 'weather'], required: true, label: 'Jenis sensor' },
  ...
  incidents: {
    ...
    type: { type: 'enum', values: ['peringatan', 'hujan_tinggi', 'verifikasi', 'offline', 'normal'], required: true, label: 'Jenis' },
    district: { type: 'string', required: true, label: 'Kecamatan' },
    location: { type: 'string', required: true, max: 160, label: 'Lokasi' },
