...
import { SURNENEP_CENTER, type MapArea, type MapMarker, type TileMode } from '~/types/map'
...
function iconHtml(m: MapMarker): { html: string; size: [number, number]; anchor: [number, number] } {
  ...
  if (props.variant === 'peta') {
    const style = m.kind === 'district' ? { bg: STATUS_BG[m.status], icon: statusMeta(m.status).icon } : KIND_STYLE[m.kind]!
    const alert = m.kind === 'camera' && (m.status === 'bahaya' || m.status === 'siaga')
    const labelHtml = label ? `<div class="map-pin-label">${label}</div>` : ''
    const small = m.kind !== 'district'
    return {
      html: `<div class="map-pin"><div class="map-pin-icon ${small ? 'map-pin-sm' : ''} ${alert ? STATUS_BG[m.status] : style!.bg}"><i class="fa-solid ${style!.icon}"></i></div>${labelHtml}</div>`,
  ...
  if (m.kind === 'district') {
    ...
    return {
      html: `<div title="${label}" class="${size} ${STATUS_BG[m.status]} rounded-full border-2 border-white flex items-center justify-center text-white"><i class="fa-solid ${statusMeta(m.status).icon}"></i></div>`,
  ...
  return {
    html: `<div title="${label}" class="w-4 h-4 ${s.bg} ${shape} border-2 border-white shadow flex items-center justify-center text-[8px] text-white"><i class="fa-solid ${s.icon}"></i></div>`,
    size: [16, 16],
    anchor: [8, 8],
  }
}
...
function renderMarkers() {
  ...
  for (const m of props.markers) {
    ...
    const marker = L.marker([m.lat, m.lng], {
      ...
      zIndexOffset: m.kind === 'district' ? 1000 : 0,
    })
...
function renderAreas() {
  ...
  for (const a of props.areas) {
    if (props.showRiskAreas) {
      const circle: Circle = L.circle([a.lat, a.lng], {
        radius: 2200 + (a.status === 'bahaya' ? 1400 : a.status === 'siaga' ? 900 : 400),
        color: STATUS_HEX[a.status],
        weight: 0,
        fillOpacity: a.status === 'normal' ? 0.12 : 0.22,
        interactive: false,
      })
      circle.addTo(areaLayer)
    }
    if (props.showBoundaries) {
      L.circle([a.lat, a.lng], { radius: 4200, color: '#94a3b8', weight: 1, dashArray: '4 4', fill: false, interactive: false }).addTo(boundaryLayer)
    }
  }
}
...
onMounted(async () => {
  ...
  if (!el.value) return
  map = L.map(el.value, { zoomControl: false, attributionControl: true, center: SURNENEP_CENTER, zoom: props.zoom })
...
defineExpose({
  zoomIn: () => map?.zoomIn(),
  zoomOut: () => map?.zoomOut(),
  resetView: () => map?.flyTo(SURNENEP_CENTER, props.zoom, { duration: 0.8 }),
  focus(id: string, zoom = 14) {
